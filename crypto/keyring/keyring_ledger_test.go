...
func TestInMemoryCreateLedger(t *testing.T) {
	...
	k, err := kb.SaveLedgerKey("some_account", hd.Secp256k1, "bcna", 118, 3, 1)
	if err != nil {
		require.Error(t, err)
...
func TestSignVerifyKeyRingWithLedger(t *testing.T) {
	...
	k, err := kb.SaveLedgerKey("key", hd.Secp256k1, "bcna", 118, 0, 0)
	if err != nil {
		require.Equal(t, "ledger nano S: support for ledger devices is not available in this executable", err.Error())
		t.Skip("ledger nano S: support for ledger devices is not available in this executable")
		return
...
func TestAltKeyring_SaveLedgerKey(t *testing.T) {
	...
	_, err = kr.SaveLedgerKey("key", notSupportedAlgo{}, "bcna", 118, 0, 0)
	require.True(t, errors.Is(err, ErrUnsupportedSigningAlgo))

	k, err := kr.SaveLedgerKey("some_account", hd.Secp256k1, "bcna", 118, 3, 1)
	if err != nil {
		require.Equal(t, "ledger nano S: support for ledger devices is not available in this executable", err.Error())
		t.Skip("ledger nano S: support for ledger devices is not available in this executable")
		return
...
func TestSignWithLedger(t *testing.T) {
	...
	privA, _, err := ledger.NewPrivKeySecp256k1(*pathA, "bcna")
	require.NoError(t, err)
