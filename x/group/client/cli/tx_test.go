...
func (s *CLITestSuite) SetupSuite() {
	...
	_, err = clitestutil.MsgSendExec(
		...
		val.Address,
		account,
		sdk.NewCoins(sdk.NewCoin("stake", sdkmath.NewInt(2000))), address.NewBech32Codec("bcna"), fmt.Sprintf("--%s=true", flags.FlagSkipConfirmation),
		fmt.Sprintf("--%s=%s", flags.FlagBroadcastMode, flags.BroadcastSync),
		fmt.Sprintf("--%s=%s", flags.FlagFees, sdk.NewCoins(sdk.NewCoin("stake", sdkmath.NewInt(10))).String()),
	)
	s.Require().NoError(err)
...
func (s *CLITestSuite) TestTxUpdateGroupPolicyDecisionPolicy() {
	...
	cmd := groupcli.MsgUpdateGroupPolicyDecisionPolicyCmd(address.NewBech32Codec("bcna"))
	cmd.SetOutput(io.Discard)
