...
func (suite *KeeperTestSuite) SetupTest() {
	...
	suite.accountKeeper.EXPECT().AddressCodec().Return(codecaddress.NewBech32Codec("bcna")).AnyTimes()
...
func (suite *KeeperTestSuite) TestKeeperCrud() {
	...
	accAddr, err := codecaddress.NewBech32Codec("bcna").StringToBytes(address)
	suite.Require().NoError(err)
